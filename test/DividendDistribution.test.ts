...
describe("Revenue Distribution Unit Tests", function () {
  async function deployMockFixture() {
    ...
    const MockOwnershipStaking = await ethers.getContractFactory(
      "MockOwnershipStaking"
    );
    const staking = await MockOwnershipStaking.deploy();
    await staking.waitForDeployment();

    // 3. Deploy Revenue (Connected to MockOwnershipStaking)
    const Revenue = await ethers.getContractFactory("DividendDistribution");
  ...
  describe("startNewRound function", function () {
    // Permission Check (Unhappy Path)
    it("Should revert if non-owner tries to start round", async function () {
      const { revenue, attacker } = await networkHelpers.loadFixture(
        deployMockFixture
      );

      await expect(
        revenue.connect(attacker).startNewRound()
      ).to.be.revertedWithCustomError(revenue, "OwnableUnauthorizedAccount");
    });

    // Logic Check: No Stakers (Unhappy Path)
    it("Should revert if totalStaked is 0 (No Stakers)", async function () {
      const { revenue, owner, staking, alice, bob } =
        await networkHelpers.loadFixture(deployMockFixture);
      ...
      await expect(
        revenue.connect(owner).startNewRound()
    ...
    it("Should calculate reward correctly and activate new round", async function () {
      const { revenue, owner, usdt } = await networkHelpers.loadFixture(
        deployMockFixture
      );
      ...
      // Formula: (Balance * 1e18) / TotalStaked
      // Expected RPS = 5e18
      const expectedRPS = (revenueAmount * PRECISION) / totalStaked;

      // --- Execution & Verification ---
      // Verify event arguments
      await expect(revenue.connect(owner).startNewRound())
        .to.emit(revenue, "RoundStarted")
        .withArgs(1n, revenueAmount, expectedRPS / PRECISION);
    ...
    describe("variable : currentRoundRewardPerShare", function () {
      it("Should calculate and update currentRoundRewardPerShare correctly", async function () {
        ...
        // Verify result
        expect(await revenue.currentRoundRewardPerShare()).to.equal(
          expectedRPS
        );
      });
    });
  ...
  describe("claim function", function () {
    it("Should revert if round is not active (Round Not Started)", async function () {
      const { revenue, alice } = await networkHelpers.loadFixture(
        deployMockFixture
      );
    ...
    it("Should revert if user is NOT Profitable", async function () {
      const { revenue, owner, bob, usdt } = await networkHelpers.loadFixture(
        deployMockFixture
      );
      await usdt.mint(revenue.target, 1000n);
      // 1. Start Round
      ...
      await expect(revenue.connect(bob).claim()).to.be.revertedWithCustomError(
        revenue,
        "NoProfitableRight"
      );
    });
    ...
    it("Should handle state changes dynamically", async function () {
      const { revenue, staking, bob, owner, usdt } =
        await networkHelpers.loadFixture(deployMockFixture);
      await usdt.mint(revenue.target, 1000n);
      await revenue.connect(owner).startNewRound();
    ...
    it("Should revert if user tries to claim twice in the same round", async function () {
      ...
      const { revenue, staking, alice, owner, usdt } =
        await networkHelpers.loadFixture(deployMockFixture);
      ...
      await expect(revenue.connect(alice).claim()).to.emit(
        revenue,
        "RewardClaimed"
      );
      ...
      await expect(
        revenue.connect(alice).claim()
      ).to.be.revertedWithCustomError(revenue, "DoubleClaimed");
    });
  });

  describe("checkCurrentRoundReward function", function () {
    // 1. Happy Path: Check Reward
    it("Should return correct amount for a profitable user in an active round", async function () {
      const { revenue, owner, alice, bob, usdt, staking } =
        await networkHelpers.loadFixture(deployMockFixture);
    ...
    it("Should return 0 if round is not active", async function () {
      const { revenue, alice, staking, usdt } =
        await networkHelpers.loadFixture(deployMockFixture);
    ...
    it("Should return 0 if user is not profitable", async function () {
      const { revenue, owner, bob, usdt, staking } =
        await networkHelpers.loadFixture(deployMockFixture);
    ...
    it("Should return 0 if user has already claimed this round", async function () {
      ...
      // First check: 1000
      expect(await revenue.checkCurrentRoundReward(alice.address)).to.equal(
        1000n
      );
  ...
  describe("transferRevenueOut function", function () {
    it("Should revert if target address is zero", async function () {
      const { revenue, owner } = await networkHelpers.loadFixture(
        deployMockFixture
      );

      await expect(
        revenue.connect(owner).transferRevenueOut(ethers.ZeroAddress, 100n)
    ...
    it("Should revert if contract balance is less than amount", async function () {
      const { revenue, owner, alice } = await networkHelpers.loadFixture(
        deployMockFixture
      );

      await expect(
        revenue.connect(owner).transferRevenueOut(alice.address, 100n)
    ...
    it("Should revert if USDT transfer returns false (MockBadToken)", async function () {
      ...
      await expect(
        badRevenue.connect(owner).transferRevenueOut(alice.address, 100n)
    ...
    it("Should revert if non-owner tries to transfer USDT", async function () {
      const { revenue, attacker, alice } = (await networkHelpers.loadFixture(
        deployMockFixture
      )) as any;

      await expect(
        revenue.connect(attacker).transferRevenueOut(alice.address, 100n)
      )
        .to.be.revertedWithCustomError(revenue, "OwnableUnauthorizedAccount")
    ...
    it("Should allow owner to transfer USDT out of contract", async function () {
      ...
      await expect(
        revenue.connect(owner).transferRevenueOut(alice.address, 500n)
      )
